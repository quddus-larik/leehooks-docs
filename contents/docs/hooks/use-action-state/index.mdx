 ...
 */
function useActionState(initialStatus: Status = 'idle'): ActionStateAPI
```
...
| :--- | :--- | :--- |
| `ActionStateRenderer` | **Unified, Low-Code** | A single component that accepts `idle`, `pending`, `success`, and `error` props to map the entire state flow declaratively. |
...
### 1. Example Without useActionState
...
### 2. Example With useActionState

This approach uses the unified `ActionStateRenderer` for a declarative, low-code component structure. All state management and conditional rendering logic are delegated to the hook.

```javascript
import React from 'react';
// import { useActionState } from './useActionState'; // Assume hook import

// NOTE: useActionState and fetchUser are assumed to be available.
